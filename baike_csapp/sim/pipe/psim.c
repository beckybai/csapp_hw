...
int instr_limit = (1 << 31) - 1; /* Instruction limit [TTY only] (-l) */
...
//    byte_t* ptrr;
    ...
    if ( ( ptrr = mmap(NULL, SHARE_SIZE, \
     PROT_READ | PROT_WRITE, MAP_SHARED| MAP_FILE, fd, 0)) == MAP_FAILED) {
    printf("mmap error: %s\n", strerror(errno));
    exit(EXIT_FAILURE);
    }
    int j;
...
*/
	...
	strcpy(myargv[1], "~/Documents/csapp/sim/pipe/pipe.tcl");
...
static void run_tty_sim() 
{
	...
	printf("free register memory");
	free_mem(isa_state->m);
	isa_state->m = copy_mem(mem);
	printf("copy check regester begin");
	isa_state->r = copy_mem(reg);
	isa_state->cc = cc;
	printf("copy chech regester end");
    }

    printf("psim:copy memory begin\n");
    mem0 = copy_mem(mem);
    printf("psim:copy memory end\n");
    printf("psim:copy regester begin");
    reg0 = copy_mem(reg);
    printf("psim:copy register end");
...
void sim_init()
{
    ...
    initialized = 1;
    printf("init memory begin\n");
    mem = init_mem(MEM_SIZE,1);
    reg = init_reg();
...
void do_mem_stage()
{
	...
	if (!dmem_error)
	  sim_log("\t%d:Memory: Read 0x%x from 0x%x\n",
		  id,valm, mem_addr);
    }
    if (mem_write) {
	word_t sink;
	/* Do a read of address just to check validity */
	printf("here?the mem_addr is %d\n",mem_addr);
